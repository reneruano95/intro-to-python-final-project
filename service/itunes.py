...
async def get_tracks(album: Album) -> None:
    params: dict[str, str | int] = {
        "id": album.id,
        "entity": "song"
    }
...
@cache_artist
async def search_artist(artist_name: str, limit: int) -> Artist:
    artist = await get_artist(artist_name, limit)
    for album in artist.albums:
        await get_tracks(album)
    return artist
